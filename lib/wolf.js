...
Wolf.prototype._get = function(url, callback) {
  var getURL = this.config.endpoint + '/' + url,
      success = new RegExp("^2\d\d$"); // Construct URL with parameters

  request.get({
    url: getURL,
    json: true
  }, function(error, response, body) {
    if (!error && success.test(response.statusCode)) {
